...
function ApplyButton({ job, user, isClosed }: { job: any, user: any, isClosed: boolean }) {
    const { verifyAccess } = useProtectedAction();
    const [open, setOpen] = useState(false);

    // Check if already applied
    const existingApplication = useQuery(api.applications.getMyApplication, { jobId: job._id });
    ...
    const handleApply = () => {
        if (verifyAccess("apply for this job")) {
            // New Flow: Always open modal. It handles pre-fill and validation.
            setOpen(true);
        }
    };
    ...
    if (isClosed) {
        return (
            <Button
                className="w-full font-semibold h-12 rounded-lg text-lg bg-slate-300 text-slate-500 cursor-not-allowed hover:bg-slate-300"
                disabled
            >
                Applications Closed
            </Button>
    ...
    return (
        <>
            <Button
                className={`w-full font-semibold h-12 rounded-lg text-lg ${isApplied ? "bg-green-600 hover:bg-green-700" : "bg-blue-600 hover:bg-blue-700"} text-white`}
                onClick={handleApply}
                disabled={isApplied}
            >
                {isApplied ? (
...
export function JobDetailsContent({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    const job = useGetJob(id);
    const user = useGetUser();

    const trackView = useMutation(api.user_actions.trackView);
            ...
            <div className="container mx-auto px-4 max-w-6xl">
                ...
                <div className="relative bg-white rounded-xl p-8 shadow-sm border border-slate-100 mb-8 overflow-hidden min-h-[200px] flex items-center">
                    ...
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 opacity-[0.03] select-none pointer-events-none">
                        {job.logoUrl ? (
                            <img src={job.logoUrl} alt="" className="h-[400px] w-auto grayscale" />
                        ) : (
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start w-full">
                        ...
                        <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl border bg-white shadow-sm flex items-center justify-center shrink-0 p-4">
                            {job.logoUrl ? (
                                <img src={job.logoUrl} alt={job.company?.name || "Company Logo"} className="w-full h-full object-contain" />
                            ) : (
                                <span className="text-4xl font-bold text-slate-700">{job.company?.name?.[0] || "C"}</span>
                            )}
                        </div>
                        ...
                        <div className="flex-1 space-y-4">
                            <div>
                                ...
                                {(job.status === "closed" || (job.applicationDeadline && job.applicationDeadline < Date.now())) && (
                                    <div className="mt-2 inline-flex">
                                        <Badge variant="destructive" className="text-sm px-3 py-1">Applications Closed</Badge>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-xl p-2 shadow-sm flex gap-2 overflow-x-auto sticky top-4 z-20 border border-slate-100">
                            {[
                                ...
                                { label: "Job Description", id: "job-description" },
                                { label: "Dates & Deadlines", id: "dates-deadlines" },
                        ...
                        {/* Job Description */}
                        <Card id="job-description" className="border-none shadow-sm scroll-mt-20">
                            <CardContent className="p-6">
                    ...
                    <div className="space-y-6">
                        ...
                        <Card className="border-none shadow-sm">
                            <CardContent className="p-6">
                                ...
                                <ApplyButton job={job} user={user} isClosed={job.status === "closed" || (job.applicationDeadline ? job.applicationDeadline < Date.now() : false)} />
                            </CardContent>
                        </Card>
                            ...
                            <CardContent className="p-0 divide-y">
                                ...
                                <div className="p-4 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className={`font-bold ${getDaysLeft(job.applicationDeadline) && getDaysLeft(job.applicationDeadline)! < 5 ? 'text-red-600' : 'text-slate-900'}`}>
                                            {getDaysLeft(job.applicationDeadline) !== null ? `${getDaysLeft(job.applicationDeadline)} Days Left` : "Open"}
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        {job.workDays && (
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="font-bold text-slate-900">{job.workDays}</div>
                                                </div>
                                            </div>
                                        )}
                                        {job.workHours && (
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="font-bold text-slate-900">{job.workHours}</div>
                                                </div>
                                            </div>
                                        )}
                                    </div>
