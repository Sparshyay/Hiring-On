...
export function EditContent() {
    ...
    const isFromOnboarding = searchParams.get("from_onboarding") === "true";

    useEffect(() => {
        if (isFromOnboarding) {
            toast.info("Please review your details and click 'Save Changes' to confirm your profile.");
        }
    }, [isFromOnboarding]);
    ...
    const handleSave = async (dataToSave?: any) => {
        ...
        if (cleanedData.domainId === "") cleanedData.domainId = undefined;
        if (cleanedData.courseId === "") cleanedData.courseId = undefined;
        if (cleanedData.specializationId === "") cleanedData.specializationId = undefined;
        ...
        try {
            ...
            // Determine if this is a confirmation action or just a regular save
            const isConfirmation = isFromOnboarding || (convexUser as any)?.onboardingStatus === "pending";
            ...
            if (isConfirmation) {
                payload = {
                    ...
                    profileStatus: {
                        autoFilled: (convexUser as any)?.profileStatus?.autoFilled || false,
                        confirmed: true,
                        completionLevel: 3
                    }
                };
            ...
            toast.success("Profile confirmed and updated!");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-3 flex flex-col items-center">
                <div className="relative group">
                    ...
                    {(() => {
                        const calculateProgress = () => {
                            ...
                            const checkField = (field: any) => {
                                ...
                                if (Array.isArray(field)) {
                                    if (field.length > 0) filledFields++;
                                } else if (typeof field === 'object' && field !== null) {
                                    if (Object.values(field).some(val => val !== "")) filledFields++;
                                } else {
                                    if (field && field.toString().trim() !== "") filledFields++;
                                }
                            };
                            checkField(formData.firstName);
                            checkField(formData.lastName);
                            checkField(formData.mobile);
                            checkField(formData.currentLocation);
                            checkField(formData.domainId);
                            checkField(formData.courseId);
                            checkField(formData.about);
                            checkField(formData.skills);
                            checkField(formData.education);
                            return totalFields === 0 ? 0 : Math.round((filledFields / totalFields) * 100);
                        };
                        const progress = calculateProgress();
                        const radius = 70; // Slightly larger than avatar (w-32 = 128px / 2 = 64px) -> radius 70 for ring
                        ...
                        return (
                            <div className="relative flex items-center justify-center">
                                {/* Background Circle */}
                                <svg className="absolute w-[160px] h-[160px] rotate-[-90deg]" viewBox="0 0 160 160">
                                    <circle
                                        cx="80"
                                        cy="80"
                                        r="70"
                                    ...
                                    <circle
                                        ...
                                        r="70"
                                        fill="transparent"
                                        className={cn("transition-all duration-1000 ease-out", progress === 100 ? "stroke-green-500" : "stroke-blue-600")}
                                        strokeWidth="6"
                                ...
                                <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-lg bg-primary/10 flex items-center justify-center relative z-10">
                                ...
                                {/* Percentage Badge */}
                                <div className={cn(
                                    "absolute -bottom-3 z-20 px-3 py-1 rounded-full text-xs font-bold border-2 border-white shadow-md transition-colors",
                                    progress === 100 ? "bg-green-500 text-white" : "bg-blue-600 text-white"
                                )}>
                                    {progress}% Profile
                                </div>
                            </div>
                    ...
                    <div className="absolute top-0 right-0 bg-white p-2 rounded-full shadow-md border cursor-pointer hover:bg-slate-50 transition-colors z-20 translate-x-1/2 translate-y-1/2 hidden">
                        {/* Hidden camera icon since overlay handles it or just keep it minimal */}
    ...
    return (
        <div className="min-h-screen bg-slate-50">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <Button onClick={() => handleSave()} disabled={isSaving} className="rounded-full bg-blue-600 hover:bg-blue-700 text-white px-6">
                                {isSaving ? "Saving..." : ((isFromOnboarding || (convexUser as any)?.onboardingStatus === "pending") ? "Confirm Profile" : "Save Changes")}
                            </Button>
                        </div>
                    </div>
                </div>
            ...
            {/* Confirmation Banner for Onboarding Users */}
            {(isFromOnboarding || (convexUser as any)?.onboardingStatus === "pending") && (
                <div className="bg-orange-50 border-b border-orange-200 p-4 sticky top-[153px] z-30">
                    <div className="container mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-3 text-orange-800">
                            <Info className="w-5 h-5" />
                            <p className="font-medium text-sm">
                                Action Required: Please review your auto-filled details and click "Save/Confirm Profile" to complete onboarding.
                            </p>
                        </div>
                        <Button size="sm" onClick={handleSave} className="bg-orange-600 hover:bg-orange-700 text-white border-none">
                            Confirm Profile
                        </Button>
                    </div>
                </div>
            ...
            <div className="container mx-auto px-4 py-8">
                <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
                    ...
                    <div className="hidden lg:block w-72 shrink-0">
                        <div className="sticky top-40 space-y-6 max-h-[calc(100vh-8rem)] overflow-y-auto custom-scrollbar">
                            {/* Progress Card */}
                            {(() => {
                                const calculateProgress = () => {
                                    let totalFields = 0;
                                    ...
                                    const checkField = (field: any) => {
                                        ...
                                        if (Array.isArray(field)) {
                                            ...
                                            // Handle nested objects like courseDuration or addresses if needed
                                            // For simplicity, just checking if it's not empty/null
                                            if (Object.values(field).some(val => val !== "")) filledFields++;
                                        } else {
                                            if (field && field.toString().trim() !== "") filledFields++;
                                        }
                                    ...
                                    // Define required fields to track for "Required" badges and overall progress
                                    // Basic
                                    checkField(formData.firstName);
                                    checkField(formData.lastName);
                                    checkField(formData.mobile);
                                    checkField(formData.currentLocation);
                                    checkField(formData.domainId); // Domain
                                    checkField(formData.courseId); // Course

                                    // Resume - Optional but good to have
                                    // checkField(formData.resume);
                                ...
                                return (
                                    <div className="bg-slate-50 border rounded-xl p-4">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span className={cn("font-bold text-sm", progress === 100 ? "text-green-600" : "text-blue-600")}>{progress}%</span>
                                        </div>
                                        <p className="text-[11px] text-muted-foreground mb-3 leading-relaxed">
                                            {progress === 100 ? "Great job! Your profile is complete." : "Stay ahead of the competition by regularly updating your profile."}
                                        </p>
                                        <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                            <div
                                                className={cn("h-full transition-all duration-500 rounded-full", progress === 100 ? "bg-green-500" : "bg-blue-500")}
                                    ...
                                    >
                                        ...
                                        {(() => {
                                            const isFilled = (id: string) => {
                                                switch (id) {
                                                    case "basic": return formData.firstName && formData.mobile && formData.currentLocation && formData.domainId;
                                                    case "about": return formData.about;
                                                    case "skills": return formData.skills && formData.skills.length > 0;
                                                    case "education": return formData.education && formData.education.length > 0;
                                                    default: return true; // Other sections optional
                                                }
                                            };
                                            const showRequired = section.required && !isFilled(section.id);
            ...
            <Dialog open={isReviewOpen} onOpenChange={setIsReviewOpen}>
                <DialogContent className="max-w-2xl bg-white p-0 gap-0 overflow-hidden focus:outline-none z-[9999] max-h-[90vh] flex flex-col">
                            ...
                            onClick={async () => {
                                ...
                                const newData = {
                                    ...
                                    education: (parsedData.education && parsedData.education.length > 0) ? parsedData.education.map((e: any) => ({
                                        ...
                                        college: e.institution || e.college || e.university || "",
